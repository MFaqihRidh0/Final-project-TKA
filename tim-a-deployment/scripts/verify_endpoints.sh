...
API="${BASE}/api"        # semua endpoint Flask lewat /api/ prefix (Nginx proxy)
...
PRODS=$(curl -s "$API/products?limit=1")
...
ORDER=$(curl -s -X POST "$API/orders" "${AUTH[@]}" -H 'Content-Type: application/json' \
...
code=$(curl -s -o /dev/null -w '%{http_code}' "$API/orders/$OID" "${AUTH[@]}")
check "get order detail" 200 "$code"

echo "── 5. PUT /orders/<id>/status (admin → processing)"
code=$(curl -s -o /dev/null -w '%{http_code}' -X PUT "$API/orders/$OID/status" "${AUTH[@]}" \
  -H 'Content-Type: application/json' -d '{"status":"processing"}')
check "update status" 200 "$code"

echo "── 6. GET /orders (list)"
code=$(curl -s -o /dev/null -w '%{http_code}' "$API/orders?limit=5" "${AUTH[@]}")
